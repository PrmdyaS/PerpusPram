...
class PeminjamanController extends GetxController {
  ...
  var responseBorrowBook = <BorrowBook>[].obs;
  var id = "".obs;
  ...
  getData() async {
    loading(true);
    try {
      final response = await ApiProvider.instance().get("${Endpoint.borrowBooks}/users/${id.value}");
      if (response.statusCode == 200) {
        final List<BorrowBook> responseData = (response.data['data'] as List)
            .map((json) => BorrowBook.fromJson(json))
            .toList();
        responseBorrowBook.assignAll(responseData);
      } else {
